...
public class MarqueeText extends TextView implements Runnable {
	...
	public void run() {
		...
		if (scrollDirection == SCROLL_TO_LEFT) {
			currentScrollX += SCROLL_OFFSET_X;
			scrollTo(currentScrollX, 0);
			if (getScrollX() >= textWidth + pianyi) {
				scrollTo(-mViewWidth + pianyi, 0);
				currentScrollX = -mViewWidth + pianyi;
			}
		} else {
			// 来回移动待实现
		}
		...
		/*if (scrollDirection == SCROLL_TO_LEFT) {
			currentScrollX += SCROLL_OFFSET_X;
			scrollBy(SCROLL_OFFSET_X, 0);
			if (currentScrollX >= textWidth + pianyi) {
				scrollTo(-mViewWidth + pianyi, 0);
				currentScrollX = -mViewWidth + pianyi;
			}
		} else {
